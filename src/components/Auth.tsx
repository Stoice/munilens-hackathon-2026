...
export default function Auth({ user, loading }: AuthProps) {
  ...
  return (
    <div className="flex items-center space-x-4">
      ...
      {/* Auth Modal */}
      {showModal && (
        <div className="fixed top-0 left-0 w-full h-screen z-[100] flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
          <div className="bg-white/70 backdrop-blur-md rounded-2xl shadow-2xl w-full max-w-md overflow-hidden animate-in zoom-in duration-300 flex flex-col max-h-[90vh]">
            ...
            <form onSubmit={handleEmailAuth} className="p-6 space-y-4 overflow-y-auto flex-1 custom-scrollbar">
              ...
              {authMode === 'signup' && (
                <div className="grid grid-cols-2 gap-4">
                  <div className="space-y-1">
                    <label className="text-[10px] font-black uppercase tracking-widest text-gray-400">{t('auth.firstName')}</label>
                  ...
                  <div className="space-y-1">
                    <label className="text-[10px] font-black uppercase tracking-widest text-gray-400">{t('auth.lastName')}</label>
                    <div className="relative">
                      <UserIcon className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-gray-400" />
                      <input 
                        required
              ...
              <div className="space-y-1">
                <label className="text-[10px] font-black uppercase tracking-widest text-gray-400">{t('auth.email')}</label>
                <div className="relative">
              ...
              <div className="space-y-1">
                <label className="text-[10px] font-black uppercase tracking-widest text-gray-400">{t('auth.password')}</label>
                <div className="relative">
              ...
              {authMode === 'signup' && (
                <>
                  <div className="space-y-1">
                    <label className="text-[10px] font-black uppercase tracking-widest text-gray-400">{t('auth.phone')}</label>
                    <input 
                      required
                      type="tel"
                      value={phoneNumber}
                  ...
                  <div className="space-y-1">
                    <label className="text-[10px] font-black uppercase tracking-widest text-gray-400">{t('auth.province')}</label>
                    <div className="relative">
                      ...
                      >
                        <option value="" disabled>{t('auth.selectProvince')}</option>
                        {PROVINCES.map(p => (
              ...
              <div className="relative py-4">
                ...
                <div className="relative flex justify-center text-[10px] uppercase font-black tracking-widest text-gray-400">
                  <span className="bg-white px-2">{t('auth.or')} continue with</span>
                </div>
              </div>
